...
def evaluate_mask(model, dataloader, mask_transform, device):
    """Evaluate AU performance with frequency mask applied"""
    correct = 0
    total = 0
    ...
    for images, (au_labels, _) in dataloader:
        ...
        correct += (preds == au_labels).sum().item()
        total += au_labels.numel()
...
def search_stage(model, dataloader, stage, num_candidates, proportion, prev_mask, device):
    ...
    baseline_acc = evaluate_mask(model, dataloader, lambda x: x, device)
    print(f"   Baseline accuracy: {baseline_acc*100:.2f}%")
    
    # Test each candidate
    results = []
    for i, mask_transform in enumerate(tqdm(candidates, desc=f"   Testing masks")):
        acc = evaluate_mask(model, dataloader, mask_transform, device)
        drop = baseline_acc - acc
        results.append((i, acc, drop, mask_transform))
    
    # Find top masks with biggest performance drop
    results.sort(key=lambda x: x[2], reverse=True)
    top_10 = results[:10]
    
    print(f"   Top mask drops: {[f'{r[2]*100:.1f}%' for r in top_10[:5]]}")
...
def main():
    ...
    dataset = BP4D_AU_dataset(args.train_json, is_train=False, args=dataset_args)
...
# python hfss_search_accuracy.py --model_type FMAE --model_path models/FMAE_BP4D_fold1.pth --num_candidates 200 --stages stage1 stage2 stage3